...
const Payment = () => {
  ...
  const { totalPrice = 0, discount = 0, payableTotal = 0 } = location.state || {};
  ...
  const handleOrderConfirm = () => {
    if (paymentMethod === 'cod') {
      toast.success('🛵ক্যাশ অন ডেলিভারির অর্ডার নিশ্চিত হয়েছে!', {
        duration: 4000,
        icon: '💸',
      });
    } else if (paymentMethod === 'wallet') {
      if (walletType && transactionId.length >= 12 && transactionId.length <= 18) {
        toast.success(`✅${walletType} পেমেন্ট সফল হয়েছে!`, {
          duration: 4000,
          icon: '📱',
        });
      } else {
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white border border-gray-300 p-4 rounded shadow space-y-4">
          <h2 className="text-lg font-semibold text-gray-700">
            Payment Method <span className="text-xs text-gray-500">(Please select a payment method)</span>
          </h2>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {[{ name: 'bKash', logo: bkashLogo }, { name: 'Nagad', logo: nagadLogo }, { name: 'Rocket', logo: rocketLogo }].map(wallet => (
                <label
                  key={wallet.name}
                  className={`flex flex-col items-center gap-1 border rounded p-2 w-24 cursor-pointer ${
                    walletType === wallet.name ? 'border-blue-500' : 'border-gray-300'
                  }`}
                >
                  <input
                    type="radio"
            ...
            {paymentMethod === 'wallet' && (
              <div className="mt-4 space-y-2">
                ...
                />
                {transactionId && (transactionId.length < 12 || transactionId.length > 18) && (
      ...
      <div className="space-y-4 bg-white p-6 border border-gray-300 rounded shadow h-fit">
        ...
        <div className="text-green-700 mt-2 text-center text-sm font-medium">
          You are saving ৳{(totalPrice + 88 - payableTotal + discount).toFixed(2)} on this order!
        </div>
      </div>
    </div>
  );
